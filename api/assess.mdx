...
<ParamField path="workload" type="AssessWorkloadOptions">
  Optional bounds for the challenge workload: `minDifficulty`, `maxDifficulty`, `minAmount`, `maxAmount`, `targetDurationMs`, and `calibration`. Used only when the action is `challenge`, but validated on every call, so an invalid workload throws even when the action would be `allow` or `block`.
</ParamField>
