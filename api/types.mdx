...
The `key` argument is whatever you passed as `context` to `createChallenge()` or `verifySolution()`, so you can bucket by IP, session, user ID, or route. `check()` may be async, which is why `createChallenge()` returns a `Promise`.
