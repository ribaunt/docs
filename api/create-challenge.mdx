...
<ParamField path="onEvent" type="(event: RibauntEvent) => void">
  Optional telemetry hook. `createChallenge()` calls it with `{ type: 'challenge-issued', difficulty, amount }` after tokens are issued. Errors thrown by the callback are caught and ignored so telemetry cannot break challenge issuance. See the [Types reference](/api/types#ribauntevent) for the full event union.
</ParamField>
...
try {
  const challenges = await createChallenge({
    ...
    context: req.ip,
    rateLimiter: {
      check: async (key) => allowRequest(key), // your bucket / Redis check
    },
  });
  res.json({ challenges });
} catch (error) {
