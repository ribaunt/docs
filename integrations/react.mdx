...
The wrapper also binds standard handler props such as `onClick` and `onKeyDown` as native event listeners, and syncs plain HTML props like `title` and `data-*` attributes across renders. Removing one of these props from your JSX removes the corresponding attribute from the element.
