---
title: "Add Ribaunt CAPTCHA to Plain HTML Pages Without a Framework"
...
<Steps>
  ...
  Add the widget's ES module as a `<script type="module">` tag in your `<head>`. Adjust the path to match wherever you serve your `node_modules` or bundled output.

  ```html
  <script type="module" src="/path/to/ribaunt/dist/widget-browser.js"></script>
  ```

  ### Add the `<ribaunt-widget>` element

  Place the element inside your form. Use the `challenge-endpoint`, `verify-endpoint`, and `auto-verify` attributes to configure it.
  ...
  ### Listen for the `verify` event

  When the widget successfully verifies the user it fires a `verify` event. Use it to enable your submit button or proceed with your form logic.
  ...
  If verification fails — including a solve timeout — the widget fires an `error` event. Keep the submit button disabled and inform the user.
