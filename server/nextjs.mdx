...
- `rateLimiter: { check(key?): Promise<boolean> }` — Ribaunt calls this before signing tokens or validating a solution. Return `false` to reject; Ribaunt throws a `RateLimitedError` (`code: 'rate-limited'`) that you catch and translate to a 429. The `key` argument is whatever you pass as `context`, so you can bucket by IP, session, or user.
...
export async function POST(req: Request) {
  try {
    const challenges = await createChallenge({
      ...
      amount: 4,
      ttlSeconds: 60,
      context: req.headers.get('x-forwarded-for') ?? undefined,
      rateLimiter: { check: async (key) => allowRequest(key) },
      onEvent: (event) => {
