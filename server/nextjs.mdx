...
## Rate limiting and telemetry hooks

Both `createChallenge()` and `verifySolution()` accept two optional hooks:

- `rateLimiter: { check(key?): Promise<boolean> }` — Ribaunt calls this before signing tokens or validating a solution. Return `false` to reject; Ribaunt throws a `RateLimitedError` (`code: 'rate-limited'`) that you catch and translate to a 429. Key the limiter on the client IP by closing over the request. Do not pass `context` just to feed the limiter: `context` binds every token cryptographically, and `verifySolution()` must then receive the identical value or verification fails with `context-mismatch`.
- `onEvent: (event) => void` — receives `{ type: 'challenge-issued' }`, `{ type: 'verify-success' }`, or `{ type: 'verify-failure', reason, message }`. Wire it to your metrics pipeline. Exceptions from the callback are swallowed and never break the request.
...
export async function POST(req: Request) {
  ...
  try {
    const challenges = await createChallenge({
      ...
      amount: 4,
      ttlSeconds: 60,
      rateLimiter: { check: async () => allowRequest(clientIp) },
      onEvent: (event) => {
        if (event.type === 'challenge-issued') {
          metrics.increment('captcha.issued', { difficulty: event.difficulty });
        }
      },
    });
    return NextResponse.json({ challenges });
