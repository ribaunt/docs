...
- **Rate limit the challenge endpoint.** Use [`express-rate-limit`](https://www.npmjs.com/package/express-rate-limit) to cap how many challenges a single IP can request per minute. Without this, an attacker can farm tokens for offline brute-forcing. You can also plug a limiter directly into Ribaunt by passing a `rateLimiter` to `createChallenge()` and `verifySolution()` — when its `check()` resolves `false`, Ribaunt throws a `RateLimitedError` (`code: 'rate-limited'`) that you catch and return as a 429.
...
- **Bind challenge context when needed.** Use `context` in `createChallenge()` and `verifySolution()` when you want the same token to be valid only for a specific flow, such as a signup form or password reset request.
...
- **Emit lifecycle events with `onEvent`.** Pass `onEvent` to `createChallenge()` and `verifySolution()` to receive `challenge-issued`, `verify-success`, and `verify-failure` events. `verify-failure` reuses the `VerifyFailureReason` union, so you can label metrics or logs by reason.
