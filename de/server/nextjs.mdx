...
<Tabs>
  <Tab title="challenge/route.ts">
    ...
    `createChallenge({ difficulty: 5, amount: 4, ttlSeconds: 60 })` erzeugt 4 signierte Challenge-Tokens mit Schwierigkeit 5, jeweils 60 Sekunden gültig. Die Funktion gibt ein Array von JWT-Strings zurück; das Einpacken in `{ challenges }` entspricht dem Antwortvertrag, den das Ribaunt-Widget erwartet.
  </Tab>
  <Tab title="verify/route.ts">
    ...
    `verifySolution` validiert die JWT-Signatur, prüft, dass jeder Hash die erforderliche Schwierigkeit erfüllt, lehnt abgelaufene Tokens ab und blockiert (standardmäßig) das Wiederholen von Tokens innerhalb des aktuellen Prozesses. Es gibt ein strukturiertes Ergebnisobjekt mit `valid`, `reason` und `message` zurück.
  </Tab>
</Tabs>
