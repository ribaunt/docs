...
Da der Wrapper die zugrunde liegende Web-Komponente dynamisch importiert (um SSR-Probleme zu vermeiden), gibt es einen kurzen Moment zwischen dem ersten React-Render und dem Zeitpunkt, an dem `<ribaunt-widget>` an das DOM angehängt wird. Während des SSR und des ersten Client-Renders rendert der Wrapper einen Platzhalter anstelle des Widgets, um Layout-Verschiebungen und Hydration-Flash zu vermeiden.

Standardmäßig ist der Platzhalter ein animiertes Shimmer-Skelett, das anhand der CSS-Custom-Properties des Widgets dimensioniert wird, sodass es zu den endgültigen Widget-Abmessungen passt:

- `--ribaunt-widget-height` (Standard `58px`)
- `--ribaunt-widget-width` (Standard `230px`)
- `--ribaunt-border-radius` (Standard `14px`)

Wenn du das Widget bereits mit diesen Variablen stylst, übernimmt das Skelett sie automatisch – keine zusätzliche Konfiguration nötig.

Um einen eigenen Platzhalter zu rendern – zum Beispiel einen gebrandeten Spinner, eine statische Nachricht oder gar nichts – übergib einen beliebigen React-Knoten über die `fallback`-Prop:
...
export default function MyForm() {
  return (
    <RibauntWidget
      challengeEndpoint="/api/captcha/challenge"
      verifyEndpoint="/api/captcha/verify"
      fallback={<div className="my-captcha-placeholder">Verifizierung wird geladen…</div>}
    />
  );
}
```

Übergib `fallback={null}`, um während des Ladens des Widgets gar nichts zu rendern. Der Fallback wird nur angezeigt, bis der Import der Web-Komponente abgeschlossen ist; danach ersetzt das Widget ihn und die Prop hat keine weitere Wirkung.
