...
title: "Events"
sidebarTitle: "Events"
description: "Reference for all events emitted by ribaunt-widget: verify, error, state-change, and solver-backend. Includes React callback props and detail types."
---
...
The `solver-backend` event is dispatched once per solve request when the worker selects its solving backend: `wasm` when the WebAssembly solver loaded, or `js` when WASM is disabled or unavailable. Use it for adoption telemetry or to confirm your [`wasm-mode`](/widget/configuration#wasm-solver) configuration takes effect. The detail never includes challenge contents, nonces, or hashes.
