...
<Steps>
  ...
  <Step title="Server verifies the solutions">
    ...
    Only if every check passes does `verifySolution()` return `{ valid: true }`.
...
| --- | --- | --- |
| `createChallenge({ difficulty: 4, amount: 4, ttlSeconds: 30 })` | Milliseconds | Fast / background checks |
| `createChallenge({ difficulty: 5, amount: 4, ttlSeconds: 60 })` | ~1 second | Moderate / form submission |
| `createChallenge({ difficulty: 5, amount: 8, ttlSeconds: 120 })` | ~2 seconds | High / sensitive actions |
